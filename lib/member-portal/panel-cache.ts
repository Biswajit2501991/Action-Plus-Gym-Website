...
const WORKOUT_MUSIC_PREFIX = "apg_portal_workout_music_v1_";
...
/** Workout Plan soft window. */
export const WORKOUT_PLAN_SOFT_TTL_MS = 5 * 60 * 1000;
/** Music meta (title + URL only) — longer soft window; file streams on Play. */
export const WORKOUT_MUSIC_SOFT_TTL_MS = 15 * 60 * 1000;
...
export function peekWorkoutMusicCache<T>(
  memberUuid: string,
): { data: T; savedAt: number; ageMs: number } | null {
  return peekKeyedCache<T>(WORKOUT_MUSIC_PREFIX, memberUuid);
}

export function readWorkoutMusicCache<T>(memberUuid: string): T | null {
  return peekWorkoutMusicCache<T>(memberUuid)?.data ?? null;
}

export function writeWorkoutMusicCache<T>(memberUuid: string, data: T) {
  writeKeyedCache(WORKOUT_MUSIC_PREFIX, memberUuid, data);
}
